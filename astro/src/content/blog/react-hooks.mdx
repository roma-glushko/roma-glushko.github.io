...
cover:
    image: /blog/react-hooks/agata-create-2122jrt79G8-unsplash.jpeg
    credits: Photo by <a href="https://unsplash.com/@agatacreate">Agata Create</a> on <a href="https://unsplash.com/@agatacreate">Unsplash</a>
---
...
To achieve that, we needed to implement both `componentDidMount{:.entity.name.function}` and `componentDidUpdate{:.entity.name.function}`. In order not to duplicate logic (because those methods would be almost the same), we additionally created the `fetchData(){:.entity.name.function}` method.
...
With hooks, our refactored the `PokemonInfo{:.entity.name}` component consists of three parts now:
...
The state looks like a local variable and only the `useState(){:.entity.name.function}` function
...
Overall these changes have made the `PokemonInfo{:.entity.name}` component cleaner and its structure is now more predictable and standard.
...
It's not a coincidence I paid your attention to the new `PokemonInfo{:.entity.name}` structure.
