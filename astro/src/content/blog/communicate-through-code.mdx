...
keywords:
  ...
  - python
cover:
    image: /blog/communicate-through-code/jantine-doornbos-HvYy5SEefC8-unsplash.jpg
---
...
- the function is called `remove_duplicates(){:.entity.name.function}` but it seems to omit only non-alpha-numerical values, not duplicated values
- what values are inside `tok[0]{:python}` and `tok[2]{:python}` items? How many items are there in each `tok{:python}` array?
- what's the meaning of `tok[2]{:python}` sum for each `tok[0]{:python}`?
...
Python has many data types and a good part of them are covered by syntax sugar to make them easier to use. Remaining data types can be found in the `collections{:python}` package.
...
Python goes even further and lets you name each item in the tuples via the `NamedTuple{:python}` class:
...
Even though dicts have [become ordered recently (remembers order of adding keys)](https://stackoverflow.com/questions/39980323/are-dictionaries-ordered-in-python-3-6), you may still want to consider them unordered and use a whole another `OrderedDict{:python}` class as a means of emphasizing that you need key ordering.

You can be even more specific if your values are the same type. The `defaultdict{:python}` class will help you to automatically init all non-existing keys:
...
In order to let `assert{:python}`'s work, you should not catch any `AssertionError{:python}` and let them fail as soon as possible in your development flow.
...
- do caching of some function results without modifying the function code. The built-in `functools{:python}` package contains `lru_cache{:python}` decorator that can be used to cache the output of any function.
- add some markers to the function for the following processing. Tensorflow, for example, has a [`do_not_generate_docs(){:python}`](https://github.com/tensorflow/tensorflow/blob/299cb76dd913e7bb0349a13c1165459dac4ea81e/tensorflow/tools/docs/doc_controls.py#L46) decorator that marks functions that should not be exposed in its documentation.
...
It's not clear why `search_books(){:python}` method doesn't use `somelib.client{:python}` for the search request and it falls back to low level `requests` calls. This question will be raised many times. In order to make this piece of code better, we should have left a comment explaining the astonishing part:
