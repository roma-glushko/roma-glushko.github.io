...
The notebook file has versioning via `nbformat.nbformat_minor{:.token}` attributes. 
...
The notebook format also contains extensible points in a form of `metadata` properties on the whole notebook, cell, and output levels. It's a simple key-value maps where other plugins could store additional information if needed.
...
- Raw Code Transformation
- Code Compilation & AST Transformation
- Code Execution

Storing the execution in the history
