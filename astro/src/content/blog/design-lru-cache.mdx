...
keywords:
  ...
  - python
cover:
    image: /blog/design-lru-cache/kelly-sikkema-v9FQR4tbIq8-unsplash.jpeg
    credits: Photo by <a href="https://unsplash.com/@kellysikkema">Kelly Sikkema</a> on <a href="https://unsplash.com/s/photos/draw-on-paper">Unsplash</a>
---
...
- `LRUCache(capacity: int){:python}` class should be initialized with a capacity of the storage where the capacity is simply a number of keys that storage can hold.
- `get(key: int){:python}` method which can retrieve the value by key in constant time (O(1)).
- `put(key: int, value: int){:python}` method which stores key-value pair in the cache in constant time (O(1)). If a key is already in the storage, we need to replace its value with a new one. `put()` method should be constrained by capacity value and should not exceed it. When capacity restriction is about to be reached, we need to evict the least-recently-used item to put a new key-value pair to the storage.
...
![Cache based on a hashtable](/blog/design-lru-cache/cache-based-on-hashtable.svg "Cache based on a hashmap")
...
The problem with dictionaries is that they usually don't guarantee order in which they manage keys. So we don't have a way to quickly remove least-recently-used items. We could introduce a notion of last-used timestamps for each item in the hashmap and update these timestamps during accessing the keys in `get(){:python}` method.

![Timestamps won't help us because of sequential search we need to do on the hashtable](/blog/design-lru-cache/cache-based-on-hashtable-with-timestamps.svg "Cache as a hashtable")
...
![Cache based a linked list](/blog/design-lru-cache/cache-based-on-linked-list.svg "Cache based a linked list")
...
![LRU Cache Architecture](/blog/design-lru-cache/lru-cache-architecture.svg "LRU Cache Architecture")
...
All node manipulations (e.g. `add_to_head(){:python}`, `unlink(){:python}` methods) operates in constant time and doesn't depend on the size of the linked list.
...
The `get(){:python}` and `put(){:python}` methods only rely on methods that run in constant time, so our implementation has **constant running time on average**.
...
Specifically speaking about Python, it provides `OrderedDict{:.entity.name}` data structure that helps to implement LRU cache in a much **more concise way**.
Let's take a look:
...
<Note title={`OrderedDict vs dict`} type={`info`}>
    `OrderedDict{:.entity.name}` is not exactly the same as the dict class.
    `dict{:python}` is fully implemented on the interpreter level. It does preserve the order of inserted keys, but doesn't expose the `move_to_end(){:python}` method.
    `OrderedDict{:.entity.name}` is partially implemented on the Python level (as dict and a linked list), so it incurs some memory overhead.
</Note>
