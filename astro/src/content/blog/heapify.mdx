...
cover:
    ...
    credits: Photo by <a href="https://unsplash.com/@markusspiske">Markus Spiske</a> on <a href="https://unsplash.com/">Unsplash</a>
---
...
![Max Heap](/blog/heapify/max-heap.png "Max Heap")
...
There is going to be a custom `PriorityQueue{:.entity.name}` class which takes an arbitrary array of integers, makes a heap from it and sustains the heap property during all operations.
...
`video: title: "Building a heap": /blog/heapify/heapify.mp4`
...
`video: title: "Insert a new item to the heap": /blog/heapify/insert-a-new-item-to-the-heap.mp4`
...
The key method of the heap is extracting the max element. Because of the heap property, the max element should always stay that the top of the heap. So it's as simple as call `self.items[0]{:python}` to get it.
...
Like before, the safest way is to **replace the hole with the last leaf node** (not to change the heap property in other branches of the heap). The last item is just handy to use, but it's not necessarily the next needed candidate to be at the top of the heap. To make sure, we need to run the `heapify(){:python}` method which recursively fixes all violations.

`video: title: "Extract the max element from the heap": /blog/heapify/extract-max-element-from-the-heap.mp4`
...
`video: title: "Heap Sort In Action": /blog/heapify/heap-sort.mp4`
...
In this approach, we use the root node offset trick that helps us to change the position of the root node and move from the left to right sorting the `self.items{:python}` array. The `root_idx{:python}` param is just tweaks the child calculation formula for that.

The cost of being in-place solution is breaking the heap property in `self.items{:python}` array. I would prefer the first version of the heap sort in order to avoid such a mutations to `self.items{:python}` array.
