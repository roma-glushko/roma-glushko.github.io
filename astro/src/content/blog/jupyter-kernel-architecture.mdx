...
It gets the reference for the object via `regexp`, evaluates its value and calls `keys()` method or other specific function based on that instance type. Optionally, the instance could implement the `_ipython_key_completions_(){:python}` method to override that behavior or to integrate with a custom class.
