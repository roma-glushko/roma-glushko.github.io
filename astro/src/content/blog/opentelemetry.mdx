...
In this blog post, we will dig into two big questions:
...
- how OpenTelemetry Collector works under the hood

## The Problem

With the rise of the open source community, people and organizations don’t really want to invest into proprietary protocols and standards anymore. Instead, it's mainstream to pick a widely-recognized open source project as a basis to build on top of it.
...
This idea is so compelling that at some point there were two projects, [OpenTracing](https://opentracing.io/) and [OpenCensus](https://opencensus.io/), that were trying to fill the gap. 
They were trying to compete with each other, but there was not the right context to do that as they were working on the big ubiquitous protocol idea, so it didn't make sense to try to conquer some market shares, but rather it made to consolidate their effort and came there much quicker.
...
Most context in the observability signals are going to be conveyed via `attributes{:.variable.parameter}` or tags which are essentially key-value pairs. Then, the observability backend could do some processing of these values to correplate various pieces of information.
...
If you don't want to configure a real provider, there are also `NoOpTracerProvider{:.entity.name.class}` or `NoOpMeterProvider{:.entity.name.class}` that are helpful to keep all custom instrumentations in place without a need for removing them via `if{:python}`s, for example (this is [the null object pattern](https://en.wikipedia.org/wiki/Null_object_pattern) in action).
...
- merely filter based on presence or abcence of `attributes{:.variable.parameter}` 
...
[A condition](https://docs.python.org/3/library/threading.html#condition-objects) is used to implement this elegantly.

![A Batch Queue with Timeout](/blog/opentelemetry/otel-batch-queue-with-timeout.png "A Batch Queue with Timeout")
...
So far we have been talking about trace spans processing in scope of one services, 
but the real value of traces unlocks when multiple microservices take part in a workflow and you could connect what they were doing there into a one coherent picture.
...
![The Context Propagation in a Distributed Workflow](/blog/opentelemetry/otel-context-propagation.png "The Context Propagation in a Distributed Workflow")
...
To actually propagate that information, you should instrument your clients and servers and OTEL provides a plenty of autoinstrumentors in [their registry](https://opentelemetry.io/ecosystem/registry/).
...
- `Synchronous metrics` - theres are metrics that you measure directly right in your service workflows, so you observe them as soon as the event happens (e.g. a service increases a counter metric in the user login workflow)
- `Asynchronous (or observable) metrics` - these metrics are read from "external" sources, so you just observe an aggregated or in-time statistics instead of measure the value directly (e.g. number of items in a queue given that you cannot instrument the queue directly and you could just read its size property)
...
Metric instruments don't collect data directly but rather send it to `MeasurementConsumer{:.entity.name.class}` that is a global component initiated on the <span class="orange">MeterProvider's</span> level.
...
![Aggregation Temporalities](/blog/opentelemetry/opentelemetry-temporalities.png "Aggregation Temporalities")
<div class="image-title">Aggregation Temporalities</div>

### Metric Readers

As we briefly have mentioned, metrics are collected on schedule by <span class="blue">MetricReader</span> like <span class="blue">PeriodicExportingMetricReader</span> that holds a ticker in a separate thread.
...
![The OTEL Log Adapter in Python](/blog/opentelemetry/opentelemetry-logs.png "The OTEL Log Adapter in Python")
...
<Note title={`Design Patterns in Wild`} type={`info`}>
The OTEL's LoggingHandler is a good example of applying [the adapter pattern](https://refactoring.guru/design-patterns/adapter) that allowed OpenTelemetry to have their own architecture despite a variety of logging libraries they needed to support. 
</Note>
...
While reviewing the trace, metrics, log pipelines, we briefly have mentioned <span class="blue">exporters</span> at the end of the pipelines. 
However, OpenTelemetry SDK does support some limited number of exporters like OTEL-compatible backends, Jeager or Zipkin (for traces), Prometheus (for metrics), it's recommended to send all signals out of your services as soon as possible, and does all needed signal processing in the OTEL Collector.

The OpenTelemetry Collector could be seen as another layer of indirection that protects your bussiness logic from being modified on observability backend changes as your services are exposed to the collector only. 

At the same time, the collector lets us keep the observability logic as simple as possible on the business service end. So reliable collection of obserbability data, its batching, sampling, cleaning, and finally robust exporting to various backends are done apart from your business logic.
It simply separates that concern into a new vendor-agnostic infrastructural service that carries that out for you. 

This approach is somewhat similar to having a database, a queue or an API gateway. Each of them offloads some responsibilities from your services simplifying them in the end. 
Just like in those case, deploying the collector may make your system more complex as there is one more piece to maintain.

### The Collector Architecture

The high level architecture of the OTEL Collector is somewhat similar to the SDK trace and log systems. 
The Collector consists of three uniform signal processing pipelines for each signal type.

![The OTEL Collector Architecture](/blog/opentelemetry/opentelemetry-collector-pipelines.png "The OTEL Collector Architecture")
<div class="image-title">The OTEL Collector Architecture</div>

Each pipeline, in turn, consists of three stages.

- the <span class="green">receiver</span> stage that accepts the signals from external data sources.
- the processor stage that could do all sort of preprocessing like batching, filtering, attribute reduction, etc.
- the <span class="blue">exporter</span> stage that fans out processed data into observability backends.

It's facinationg that all of the pipelines could be assembled dynamically based on [the collector's config]().

### Receivers

As its name suggests, receives accept observability signals and this how the processing pipeline gets triggered. 

Out of the box, the OTEL Collector provides HTTP- and gRPC-based OTEL receivers. 
Since the OTEL protocol utilizes the push model, the OTEL reciever just starts a regular HTTP or gRPC servers and waits for instrumented services to reach them out.

Besides the OTEL format, the collector could be configured to accept singals in [Jaeger](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/jaegerreceiver), [Zipkin](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/zipkinreceiver), AWS X-Ray, StatsD, Prometheus protocols.

Furthermore, signals could be received from message queues like [Apache Kafka](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/kafkareceiver), GCP PubSub, Azure EventHub, etc.

Additionally, not everything speaks the OTEL protocol, so a part of receivers could extract useful stats from various systems and turn them into OTEL signals. 
It includes, Redis, MongoDB, RabbitMQ, NGINX, PostgreSQL, and [many more](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver).
