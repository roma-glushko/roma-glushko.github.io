...
Just like in the case of traces, OpenTelemetry provides `MeterProvider{:.entity.name.class}` that bridge all metrics with metric exporters. 
`MeterProvider{:.entity.name.class}` creates new instances of `Meter{:.entity.name.class}`. 
...
Metric instruments don't collect data directly but rather sending it to `MeasurementConsumer{:.entity.name.class}` that is a global component inited on the `MeterProvider{:.entity.name.class}`'s level.
...
Then, you could leverage a concept of views to override the config further. Views effectively allow to specify the aggregation strategy per metric insturment and its metadata (e.g. name, attributes).
...
- Sum Aggregation - ariphmetic sum of provided data points (used by counters)
